...
## 2026-06-11 — resumable rich-format regeneration (`--upgrade`) + validator for the rich style
The `quran/` tadabbur was rewritten into a **rich «Al-Kahf» format** in `prompt.md` (thematic
title + `معلومات السورة` table + key-vocab table + axis stops with لغوية/بلاغية pauses);
001/002/007 were regenerated by hand. To roll the format across the remaining ~111 surahs
without overwriting blindly or busting the free quota:
- `contemplate.py` now stamps `prompt_version: rich-1` into front matter and has an
  **`--upgrade`** mode that regenerates only surahs NOT yet rich (detected by the marker OR
  the `## 🗂️` signature, so already-rich ones are skipped). Reuses the daily-quota stop →
  resumable; re-run until done. (Daily task → ~6 days, then quiet.)
- Wired `--upgrade` into `daily-update.ps1` so the existing Scheduled Task does the rollout
  automatically (so the task is now KEPT enabled, not disabled, until the upgrade finishes).
- `validate_quran.py` taught the rich quote styles (`> *{{...}}*`, `{..} [ref]`, inline
  citations, multi-ayah, cross-surah) and to skip HTML-comment/code-fence regions (verse
  skeletons no longer flagged): corpus mismatches **228 → 6** (rest are real — a few model
  spelling slips + non-Quran narration). Still warning-only in the daily run.
- Model is `gemini-3.5-flash`; switch `MODEL` to `gemini-3.1-pro-preview` before runs for
  deeper flagship surahs. (A/B on البقرة: rich format is more concise + cited; quote
  fidelity needs the validator as a safety net.)
